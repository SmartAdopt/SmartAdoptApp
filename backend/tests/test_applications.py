...
class TestListApplications:
    ...
    def test_list_applications_success(self, client, db_session):
        ...
        response = client.get(
            "/applications/",
            headers={"Authorization": f"Bearer {_create_admin_token()}"},
        )
        ...
        assert app["needs_manual_review"] is False
    ...
    def test_list_applications_empty(self, client, db_session):
        _create_adopter_user(db_session)
        ...
        _clear_mongo_override()
        assert response.status_code == 404
        assert "No adoption applications found" in response.json()["detail"]["message"]
