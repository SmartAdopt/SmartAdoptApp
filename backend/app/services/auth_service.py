...
def login_user(db: Session, redis_client, login_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if user.type.lower() in ["admin", "adopter"]:
        ...
        access_token = create_access_token(
            cast(int, user.user_id), cast(str, user.type)
        )
...
def oauth_login_or_register(
    ...
    if existing_user:
        ...
        if existing_user.type.lower() in ["adopter"]:
            access_token = create_access_token(
                cast(int, existing_user.user_id), str(existing_user.type)
            )
            refresh_token = create_refresh_token(
                cast(int, existing_user.user_id), str(existing_user.type)
            )
        ...
        access_token = create_access_token(cast(int, user.user_id), str(user_type))
        refresh_token = create_refresh_token(cast(int, user.user_id), str(user_type))
