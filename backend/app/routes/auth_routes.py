    ...
    try:
        ...
        if "Email already registered" in str(e):
            raise HTTPException(
                ...
                detail={"error_code": "EMAIL_EXISTS", "message": "Email already registered", "details": str(e)}
            )
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={"error_code": "VALIDATION_ERROR", "message": "Validation error", "details": str(e)}
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"error_code": "INTERNAL_ERROR", "message": "Internal server error", "details": str(e)}
        )
...
def login(login_data: LoginRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail={"error_code": "INVALID_CREDENTIALS", "message": "Invalid email or password", "details": str(e)}
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
